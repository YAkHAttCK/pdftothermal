...
function pageTemplate(opts) {
  ...
  return [
    ...
    '      max-width: 760px;',
    '    }',
    '    .cards-3, .cards-2, .article-grid, .result-grid {',
    '      display: grid;',
    '      gap: 18px;',
    '    }',
    ...
    '      .hero-grid, .cards-3, .cards-2, .article-grid, .result-grid {',
...
const allArticles = articleTitles.map(function (item) {
  return {
    slug: item[0],
    title: item[1],
    description: item[2],
      ...
      article.sections.map(function (section) {
        return [
          ...
          section.paragraphs.map(function (p) {
            return '<p style="margin-bottom:12px;">' + escapeHtml(p) + '</p>';
          }).join(''),
          '</div>'
        ].join('\n');
      }).join('\n'),
...
app.get('/articles', function (_req, res) {
  res.send(pageTemplate({
    ...
    content: [
      ...
      '    <p class="lead">Browse practical guides, comparisons, and troubleshooting articles built around 4x6 shipping label workflows.</p>',
      '  </div>',
      '</section>',
      '<section class="section">',
      '  <div class="article-grid">',
...
app.get('/faq', function (_req, res) {
  res.send(renderSimplePage(
    '/faq',
    'FAQ | PDF to Thermal',
    'Frequently asked questions about PDF to Thermal.',
    'Frequently asked questions',
    [
      '<div class="cards-2">',
      '  <div class="faq-item"><h3>What file types can I upload?</h3><p>PDF, PNG, JPG, and JPEG are supported.</p></div>',
      '  <div class="faq-item"><h3>What size is the output?</h3><p>The tool creates a 4x6 PDF intended for common thermal label printers.</p></div>',
      '  <div class="faq-item"><h3>Does it convert every page in a PDF?</h3><p>Yes. PDF uploads are converted page by page into a multi-page 4x6 PDF output.</p></div>',
      '  <div class="faq-item"><h3>Can I preview the result before downloading?</h3><p>Yes. The completion page includes an on-screen PDF preview.</p></div>',
      '  <div class="faq-item"><h3>Does Fill mode crop?</h3><p>It can. Fill mode increases page coverage and may crop outer edges slightly.</p></div>',
      '  <div class="faq-item"><h3>What does Auto Rotate do?</h3><p>It helps wide labels fit better on a portrait 4x6 page.</p></div>',
      '</div>'
    ].join('\n')
  ));
});
...
app.get('/privacy', function (_req, res) {
  res.send(renderSimplePage(
    '/privacy',
    'Privacy Policy | PDF to Thermal',
    'Privacy information for PDF to Thermal.',
    'Privacy Policy',
    [
      '<div class="cards-2">',
      '  <div class="info-card"><h3>Uploaded files</h3><p>PDF to Thermal temporarily processes files you upload in order to generate a converted 4x6 output file.</p></div>',
      '  <div class="info-card"><h3>Storage</h3><p>Uploaded files and generated outputs are stored for a limited period, then cleaned up automatically as part of normal site operation.</p></div>',
      '  <div class="info-card"><h3>Sensitive documents</h3><p>Do not upload highly sensitive, confidential, or regulated documents.</p></div>',
      '  <div class="info-card"><h3>Questions</h3><p>If you have privacy questions, contact <strong>' + escapeHtml(SUPPORT_EMAIL) + '</strong>.</p></div>',
      '</div>'
    ].join('\n')
  ));
});
...
app.get('/terms', function (_req, res) {
  res.send(renderSimplePage(
    '/terms',
    'Terms | PDF to Thermal',
    'Terms of use for PDF to Thermal.',
    'Terms of Use',
    [
      '<div class="cards-2">',
      '  <div class="info-card"><h3>Service basis</h3><p>PDF to Thermal is provided on an as-is, as-available basis.</p></div>',
      '  <div class="info-card"><h3>Uploads</h3><p>You agree not to upload unlawful content, malicious files, or material you do not have the right to process.</p></div>',
      '  <div class="info-card"><h3>Your responsibility</h3><p>You are responsible for reviewing converted output before using it for shipment or business operations.</p></div>',
      '  <div class="info-card"><h3>Support</h3><p>Questions about these terms can be directed to <strong>' + escapeHtml(SUPPORT_EMAIL) + '</strong>.</p></div>',
      '</div>'
    ].join('\n')
  ));
});
...
app.get('/contact', function (_req, res) {
  res.send(renderSimplePage(
    '/contact',
    'Contact | PDF to Thermal',
    'Contact PDF to Thermal.',
    'Contact',
    [
      '<div class="cards-2">',
      '  <div class="info-card"><h3>Email</h3><p><strong>' + escapeHtml(SUPPORT_EMAIL) + '</strong></p></div>',
      '  <div class="info-card"><h3>Best info to include</h3><p>Your file type, selected mode, and what happened after upload.</p></div>',
      '</div>'
    ].join('\n')
  ));
});

app.get('/usps-label-to-4x6', function (_req, res) {
  res.send(renderSimplePage(
    '/usps-label-to-4x6',
    'USPS Label to 4x6 | PDF to Thermal',
    'Convert a USPS shipping label into a 4x6 thermal-printer-ready PDF.',
    'Convert USPS labels to 4x6',
    '<p>Use PDF to Thermal to turn USPS labels into a cleaner 4x6 PDF for thermal printing.</p>'
  ));
});

app.get('/ups-label-to-4x6', function (_req, res) {
  res.send(renderSimplePage(
    '/ups-label-to-4x6',
    'UPS Label to 4x6 | PDF to Thermal',
    'Convert a UPS shipping label into a 4x6 thermal-printer-ready PDF.',
    'Convert UPS labels to 4x6',
    '<p>UPS labels often print more cleanly when the source file is normalized to 4x6 first.</p>'
  ));
});

app.get('/fedex-label-to-4x6', function (_req, res) {
  res.send(renderSimplePage(
    '/fedex-label-to-4x6',
    'FedEx Label to 4x6 | PDF to Thermal',
    'Convert a FedEx shipping label into a 4x6 thermal-printer-ready PDF.',
    'Convert FedEx labels to 4x6',
    '<p>FedEx labels can be easier to print when the final PDF already matches the physical label stock.</p>'
  ));
});

app.get('/amazon-return-label-to-4x6', function (_req, res) {
  res.send(renderSimplePage(
    '/amazon-return-label-to-4x6',
    'Amazon Return Label to 4x6 | PDF to Thermal',
    'Convert an Amazon return label into a 4x6 thermal-printer-ready PDF.',
    'Convert Amazon return labels to 4x6',
    '<p>Amazon return labels often arrive in awkward formats. PDF to Thermal helps convert them into cleaner 4x6 output.</p>'
  ));
});

app.get('/ebay-label-to-4x6', function (_req, res) {
  res.send(renderSimplePage(
    '/ebay-label-to-4x6',
    'eBay Label to 4x6 | PDF to Thermal',
    'Convert an eBay shipping label into a 4x6 thermal-printer-ready PDF.',
    'Convert eBay labels to 4x6',
    '<p>eBay sellers can use PDF to Thermal to create a simpler 4x6 workflow for thermal printers.</p>'
  ));
});

app.get('/etsy-label-to-4x6', function (_req, res) {
  res.send(renderSimplePage(
    '/etsy-label-to-4x6',
    'Etsy Label to 4x6 | PDF to Thermal',
    'Convert an Etsy shipping label into a 4x6 thermal-printer-ready PDF.',
    'Convert Etsy labels to 4x6',
